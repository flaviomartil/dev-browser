...
export interface DevBrowserClient {
  ...
  attachTab: (tabId: number, name?: string) => Promise<AttachResult>;
  snapshot: () => Promise<ControlledTarget[]>;
}
...
export async function connect(serverUrl = "http://localhost:9222"): Promise<DevBrowserClient> {
  ...
  return {
    ...
    async listTabs(): Promise<BrowserTab[]> {
      ...
      const data = (await res.json()) as { tabs: BrowserTab[] };
      return data.tabs;
    },

    async attachTab(tabId: number, name?: string): Promise<AttachResult> {
    ...
    async snapshot(): Promise<ControlledTarget[]> {
      const res = await fetch(`${serverUrl}/targets`);
      if (!res.ok) {
        throw new Error(`Failed to get targets: ${await res.text()}`);
      }
      const data = (await res.json()) as { targets: ControlledTarget[] };
      return data.targets;
    },
  };
}
